...
If `<kit-root>` exists and already contains `architect-orientation.md`, `executor-orientation.md`, `COPILOT-SESSION-STATE.md`, and `ONBOARDING.md`, then this repo is already initialized: tell the user so and **stop** — do not touch anything. (This is the Copilot skill, so it owns `COPILOT-SESSION-STATE.md`; the Claude command of the same name owns `CLAUDE-SESSION-STATE.md`.)
